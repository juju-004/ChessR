...
export function ConnectionStatus() {
  ...
  useEffect(() => {
    ...
    function sendPing() {
      ...
      pingTimeoutRef.current = window.setTimeout(() => setLatencyMs(null), PING_TIMEOUT_MS);
    ...
    function onDisconnect(reason: string) {
      ...
      setState(reason === 'io server disconnect' || reason === 'io client disconnect' ? 'disconnected' : 'reconnecting');
    }
  ...
  return (
    ...
    >
      <span className={`h-2 w-2 shrink-0 rounded-full ${dotColor(state, latencyMs)}`} />
