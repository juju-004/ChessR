  ...
  useEffect(() => {
    ...
    function onRematchOffered(payload: { gameId: string }) {
      notify(
        ...
        [
          { label: 'Accept', onClick: () => socket.emit('game:rematch_respond', { gameId: payload.gameId, accept: true }) },
          {
            label: 'Decline',
            variant: 'secondary',
            onClick: () => socket.emit('game:rematch_respond', { gameId: payload.gameId, accept: false }),
          },
        ],
        30_000,
      );
