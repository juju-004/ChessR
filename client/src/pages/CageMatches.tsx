...
function matchOutcomeLabel(match: CageMatch, myId: string | undefined): string {
  ...
  const iWon = (match.matchWinner === 'p1' && iAmP1) || (match.matchWinner === 'p2' && !iAmP1);
...
export function CageMatches() {
  ...
  useEffect(() => {
    if (!socket) return;
    function onSent() {
      setStatus({ message: 'Cage match invite sent — waiting for a response…', isError: false });
    }
    function onDeclined() {
      setStatus({ message: 'Your cage match invite was declined.', isError: true });
    }
    function onAccepted() {
      setStatus({ message: 'Cage match started! Check your active matches below.', isError: false });
      refreshMatches();
    }
  ...
  function addQuickLegs() {
    ...
    const additions: CageLegPlan[] = Array.from({ length: Math.max(1, quickCount) }, () => ({
  ...
  function handleSend() {
    if (!socket) return;
    if (!selectedFriend) return setStatus({ message: 'Pick a friend to challenge.', isError: true });
    if (legs.length < 2) return setStatus({ message: 'Add at least 2 legs to the match.', isError: true });
    ...
    if (wagerMode !== 'none' && wagerTokens <= 0) {
      return setStatus({ message: 'Enter a wager amount, or set wager to "No wager".', isError: true });
    }
