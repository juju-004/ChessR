  ...
  return (
    <div className="mx-auto mt-6 max-w-2xl space-y-4">
      ...
      <div className="rounded-lg border border-base-300 bg-base-200 p-5">
        <h1 className="mb-2 text-lg font-semibold text-base-content">Friend requests</h1>
        {requests.length === 0 && <p className="text-sm text-base-content/60">No pending requests.</p>}
        {requests.map((r) => (
          <div key={r._id} className="flex items-center justify-between border-b border-base-300 py-2 last:border-none">
      ...
      <div className="rounded-lg border border-base-300 bg-base-200 p-5">
        <h1 className="mb-2 text-lg font-semibold text-base-content">Friends</h1>
        <label className="mb-1 block text-sm text-base-content/60">Challenge time control</label>
        <select
          value={tcIndex}
        ...
        <label className="mb-1 block text-sm text-base-content/60">Challenge variant</label>
        <select
          value={variant}
        ...
        <label className="mb-1 block text-sm text-base-content/60">R token wager (per player)</label>
        <input
          type="number"
          min={0}
          step={1}
        ...
        {friends.length === 0 && <p className="text-sm text-base-content/60">No friends yet. Find players and add some!</p>}
        {friends.map((f) => (
          <div key={f.id} className="flex items-center justify-between border-b border-base-300 py-2 last:border-none">
            <span className="flex items-center gap-2 text-sm text-base-content">
              <span className={`inline-block h-2 w-2 rounded-full ${f.online ? 'bg-green-500' : 'bg-base-300'}`} />
              {f.username}
            </span>
            <span className="flex gap-2">
              <Link
