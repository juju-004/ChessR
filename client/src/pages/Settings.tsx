...
import { Page, Card, CardHeader, CardTitle, CardContent, Switch, Button } from '../components/ui/index.js';
...
export function Settings() {
  ...
  async function handleReset() {
    if (await confirmDialog({ title: 'Reset all settings to their defaults?', variant: 'danger', confirmLabel: 'Reset' })) {
      resetSettings();
    }
  }
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-[1fr_240px]">
        <div className="space-y-4">
          <Card variant="solid">
            ...
            <CardContent>
              {isInstalled ? (
                <p className="text-sm text-green-500">✓ Installed — you're already running Chess App standalone.</p>
              ) : (
                <>
              ...
              <div className="flex flex-wrap gap-2">
                {PIECE_THEMES.map((t) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-base-content/50">{t.description}</div>
                  </button>
                ))}
              </div>
            </CardContent>
      ...
      <p className="mt-4 text-xs text-base-content/40">
        These preferences are saved on this device only (defaults: {DEFAULT_SETTINGS.boardTheme} board, animation{' '}
